"""
L=400 BFS measurement, one seed at a time, saves to JSON.

Run multiple times with different seeds to accumulate stats.
Usage: python3 36_L400_one_seed.py <seed>
"""
import sys
import numpy as np
import json
...
OUT_FILE = DATA / "36_L400_seeds.json"
...
def main():
    ...
    print(f"  d_BFS = {d:.5f}  (time = {elapsed:.1f}s)")
    ...
    if OUT_FILE.exists():
        ...
        data = {"L": L, "seeds": []}
    data["seeds"].append({"seed": seed, "d_BFS": d, "time_s": elapsed})
    OUT_FILE.write_text(json.dumps(data, indent=2))
    print(f"  Saved to {OUT_FILE}")
    print(f"  Total seeds collected: {len(data['seeds'])}")
    if len(data['seeds']) >= 2:
        ds = [s['d_BFS'] for s in data['seeds']]
        mean = np.mean(ds)
        se = np.std(ds, ddof=1) / np.sqrt(len(ds))
        print(f"  Mean d_BFS = {mean:.5f} ± {se:.5f}")
