...
give matches within $|{\rm ppm}| < 0.5$. The loop-effective
candidate $\varepsilon_{\rm loop} = 1/(\pi\sqrt{6})$ is
\emph{not} included in this table because it belongs to a
different interpretation of $d_{\rm eff}$: it identifies
$\varepsilon$ with the Wilson-loop self-energy dimension
$d_{\rm loop}$, not the bulk BFS dimension $d_{\rm BFS}$. It
is discussed separately in Sec.~\ref{sec:loop-eff}. The use
of $V_{d_{\rm eff}}$ rather than $V_3$ is structurally
required because the body diagonal lives in the substrate,
whose effective dimension is fractional
...
\begin{tabular}{lccc}
\toprule
candidate & dimension type & $\varepsilon$ & ppm match\\
\midrule
loop-effective & $d_{\rm loop}$ & $1/(\pi\sqrt{6})$ & $+0.014$\\
\bottomrule
\end{tabular}
\caption{Loop-effective candidate. Listed separately from the
bulk-BFS candidates of Table~\ref{tab:alpha-three-levels}
because it identifies $\varepsilon$ with the Wilson-loop
self-energy dimension $d_{\rm loop}$, not the substrate's
bulk BFS expansion dimension $d_{\rm BFS}$. Its near-exact
match becomes meaningful only if $d_{\rm loop}$ is selected
...
CODATA at $-0.38$~ppm (for the bulk-BFS candidate
$\varepsilon = 1/(2\pi)$). We have presented Maxwell-like
